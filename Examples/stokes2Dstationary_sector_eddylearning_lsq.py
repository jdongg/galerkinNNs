import numpy as np
from Formulations.stokes2D.galerkinNN_stokes2Dstationary_H2weighted_eddylearning import GNN
import sys
np.set_printoptions(threshold=sys.maxsize)
import os
from QuadratureRules.QuadratureRules import QuadratureRules

RESULTS_PATH = 'stokes2Dstationary_eddylearning'
if not os.path.exists(RESULTS_PATH):
    os.makedirs(RESULTS_PATH)

"""
This script solves the Stokes equation in 2D in a triangle wedge with
trainable singular functions (Example 4.3 from [1]). To run, execute
...
def main():
    ...
    ng = 128                 # number of quadrature nodes in shortest dimension of domain          
    ...
    QuadTrain.GaussLegendreTriangularWedge(ng)
    QuadVal = QuadratureRules()
    QuadVal.GaussLegendreTriangularWedge(150)

    nstream = 251
    x = np.linspace(-1.0, 1.0, nstream)
    y = np.linspace(-3.0, 0.0, nstream)
    X, Y = np.meshgrid(x, y)
    xStream = np.zeros([nstream**2, 2])
    xStream[:,0] = np.reshape(X, [nstream**2, ])
    xStream[:,1] = np.reshape(Y, [nstream**2, ])

    # set points outside cavity to NaN so they don't show in plots
