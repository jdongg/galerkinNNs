import numpy as np
from formulations.galerkinNN_poisson1D_H2 import GNN
import sys
np.set_printoptions(threshold=sys.maxsize)

import os

RESULTS_PATH = 'poisson1D_H2_results'
...
def exact(x):
	...
	return np.sin(2.0*np.pi*x) + 0.1 * np.sin(25.0*np.pi*x) + 5.0*np.pi*eps*(10.0*eps - 8.0*x + 9.0)/(20.0*eps+10.0)
	# return x*(1.0-x) + eps
...
def exactDx(x):
	...
	return (2.0*np.pi) * np.cos(2.0*np.pi*x) + 0.1 * (25.0*np.pi) * np.cos(25.0*np.pi*x) - 40.0*np.pi*eps/(20.0*eps+10.0)
	# return 1.0 - 2.0*x 


# second derivative of true solution
def exactDx2(x):
...
def main():
	...
	boundaryPen1 = 1.0/eps # penalty parameters for BCs
	boundaryPen2 = 1.0/eps 

	model = GNN(INIT_NEURONS, INIT_SCALE, lambda x: source(x), lambda x: exact(x),
			    lambda x: exactDx(x), lambda x: exactDx2(x), xa, xb, ng, tol, gradTol, boundaryPen1,
			    boundaryPen2, MAX_EPOCH, MAX_REFS, RESULTS_PATH)
	model.generateBasis()
