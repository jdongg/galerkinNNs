...
class GNN:
    ...
    which is always SPD. This class solves the Hemker equation
     
                ε * u''(x) + x * u'(x) = -ε * π^2 * np.cos(π * x) - π * x * np.sin(π * x)
                u(-1) = -2,   u(1) = 0

    which has exact solution u(x) = np.cos(π * x) + erf(x / sqrt(2*ε)) / erf(1.0 / nsqrt(2*ε)). 
    This solution contains a boundary layer of width sqrt(ε) at x=0. This example is meant to demonstrate
    how to formulate non-SPD problems into the least squares variational framework. Following [1], we take

                a(u,v) = (εu'' + xu', εv'' + xv')_L2(Ω) + C*(u,v)_L2(∂Ω)
                L(v)   = (f, εv'' + xv')_L2(Ω) + C*(u_D, v)_L2(∂Ω).
    ...
    def __init__(self, neurons: int, scale: np.float64, Lx: np.float64, isExtended: bool, 
                 ...
                 boundaryPen1: np.float64, boundaryPen2: np.float64,  scale_increment: np.float64, 
                 base_lr: np.float64, iter_decay_rate: np.float64, exp_decay_rate: np.float64, maxEpoch: int, 
                 maxRef: int, RESULTS_PATH: str, beta: np.float64, myeps: np.float64):
        ...
        #   0 - x
        # 
        # modifying the list contained in self.intDerivatives["u"] allows the requested
        # derivatives to be computed and accessed automatically in computeLoss, galerkinUpdate,
        # and appendBasis routines.
        self.intDerivatives = {}
    ...
    def activMatrix(self, i, k, NN, solution0,
        ...
        # interior terms
        # A matrix
        NNdxphi_idx = np.reshape(NN["u"]["interior_x"][:,i], [self.QuadRules.interior_x.shape[0], 1])
        NNdxphi_jdx = NN["u"]["interior_x"]

        NNdx2phi_idx = np.reshape(NN["u"]["interior_xx"][:,i], [self.QuadRules.interior_x.shape[0], 1])
        NNdx2phi_jdx = NN["u"]["interior_xx"]

        A[i,:] = np.sum(self.QuadRules.interior_w * (w*(self.eps * NNdx2phi_idx + xTrain * NNdxphi_idx)) * 
                                                    (w*(self.eps * NNdx2phi_jdx + xTrain * NNdxphi_jdx)), axis=0)
        ...
        # RHS
        F[i] = np.sum(self.QuadRules.interior_w * (w*DATA["F1"]) * (w*(self.eps * NNdx2phi_idx + xTrain * NNdxphi_idx)))
        F[i] += self.boundaryPen1 * np.sum(self.QuadRules.boundary_w * (wBdry*NNbdryphi_idx) * (wBdry*DATA["G1"]))
        F[i] += -np.sum(self.QuadRules.interior_w * (w*(self.eps * NNdx2phi_idx + xTrain * NNdxphi_idx)) * 
                                                    (w*(self.eps * solution0["u"]["interior_xx"] + xTrain * solution0["u"]["interior_x"])))
    ...
    def computeLoss(self, params, k, 
        ...
        normint1 = jnp.sum(jnp.multiply(W, jnp.square(jnp.multiply(resWeight, self.eps * phi["u"]["interior_xx"] + X * phi["u"]["interior_x"]))))
        ...
        r1 = jnp.sum(jnp.multiply(W, jnp.multiply(jnp.multiply(resWeight, self.eps * SOL0["u"]["interior_xx"][:,0] + X * SOL0["u"]["interior_x"][:,0]), 
                                                  jnp.multiply(resWeight, self.eps * phi["u"]["interior_xx"] + X * phi["u"]["interior_x"]))))
        ...
        r3 = jnp.sum(jnp.multiply(W, jnp.multiply(jnp.multiply(resWeight, F1), jnp.multiply(resWeight, self.eps * phi["u"]["interior_xx"] + X * phi["u"]["interior_x"]))))
    ...
    def computeError(self, solution0):
        ...
        exact1 = f
        approx1 = self.eps * solution0["u"]["interior_xx"] + xVal * solution0["u"]["interior_x"]
    ...
    def galerkinSolve(self, k):
        ...
        for idx in range(k+1):
            for jdx in range(k+1):
                A[idx,jdx] = np.sum(wGlobalFlattened * (w*(self.eps * self.basis["u"][idx]["interior_xx"] + xTrain.flatten() * self.basis["u"][idx]["interior_x"])) * 
                                                       (w*(self.eps * self.basis["u"][jdx]["interior_xx"] + xTrain.flatten() * self.basis["u"][jdx]["interior_x"])))
            ...
            F[idx] = np.sum(wGlobalFlattened * (w*(self.eps * self.basis["u"][idx]["interior_xx"] + xTrain.flatten() * self.basis["u"][idx]["interior_x"])) * 
                                                 (w*f1Train))
    ...
    def generateBasis(self):
        ...
        pickle.dump(mdic, open(self.RESULTS_PATH + "/saved_hemker1d_results.pkl", "wb"))
