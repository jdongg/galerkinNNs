...
#		   the formulation used is the H1 formulation with boundary 
#		   regularization:
#				a(u,v) = (u',v') + 1/eps*(u,v)
#				L(v) = (f,v)
#
...
#		u, ux -- function for true solution and its derivative
#		a, b -- 1D domain
...
#		boundaryPen -- 1/eps for boundary term
...
#------------------------------------------------------------------
class GNN:
	def __init__(self, neurons, scale, f, u, ux, a, b, ng, tolerance, gradTolerance, 
				 boundaryPen, maxEpoch, maxRef, RESULTS_PATH):
	...
	#------------------------------------------------------------------
	def activMatrix(self, i, NNoutput, NN_dx, NNbdry, u0x, u0bdry, K, F):

		NN_idx = np.reshape(NNoutput[:,i], self.xGlobal.shape)

		NNdx_idx = np.reshape(NN_dx[:,i], self.xGlobal.shape)
		NNdx_jdx = NN_dx
		...
		# equivalent to a(u, v)
		K[i,:] = np.sum(self.wGlobal * (NNdx_idx) * (NNdx_jdx), axis=0)
		K[i,:] += self.boundaryPen * np.sum(self.wBdry * (NNbdry_idx) * (NNbdry_jdx), axis=0)
	 
	 	# RHS: equivalent to L(v) - a(u0, v)
		F[i] = np.sum(self.wGlobal * (NN_idx) * (self.sourceFunc(self.xGlobal)))
		# F[i] += self.boundaryPen * np.sum(self.wBdry * (NNbdry_idx) * (self.exactSol(self.xBdry)))
		F[i] -= np.sum(self.wGlobal * (NNdx_idx) * (u0x))
		F[i] -= self.boundaryPen * np.sum(self.wBdry * (NNbdry_idx) * u0bdry)
	...
	#		in-place modification of K and F
	#------------------------------------------------------------------
	def galerkinUpdate(self, neurons, params, u0x, u0bdry, scale):
		...
		# evaluate derivative of network function
		gradx = jax.vmap(jax.jacfwd(self.networkArray, 0), (0, None, None), 0)
		NN_dx = jnp.squeeze(gradx(self.xGlobal, params, scale))

		# assemble matrices, columns can be done in parallel
		Parallel(n_jobs=8, backend="threading")(
			delayed(self.activMatrix)(i, NNoutput, NN_dx, NNbdry, u0x, u0bdry, K, F) for i in range(neurons))
	...
	#				  galerkinUpdate
	#		U0x -- derivative of the previous approximation u_{i-1}
	#		U0bdry -- boundary values of previous approx. u_{i-1}
	#		F -- RHS of PDE
	...
	#------------------------------------------------------------------
	@partial(jax.jit, static_argnums=(0,))
	def computeLoss(self, params, X, W, Xbdry, Wbdry, coeffs, scalek, U0x, U0bdry, F, G):
		...
		phix = jnp.reshape(gradx(X, params, coeffs, scalek), U0x.shape)

		# compute loss function
		normint = jnp.sum(jnp.multiply(W, jnp.square(phix)))
		normbdry = self.boundaryPen * jnp.sum(jnp.multiply(Wbdry, jnp.square(phibdry)))
		norm = jnp.sqrt(normint + normbdry)
		

		# a(u0, v) = (u0, v)
		r1 = jnp.sum(jnp.multiply(W, jnp.multiply(U0x, phix)))
		r2 = self.boundaryPen * jnp.sum(jnp.multiply(Wbdry, jnp.multiply(U0bdry, phibdry)))

		# L(v) = (f, v)
		r3 = jnp.sum(jnp.multiply(W, jnp.multiply(F, phi)))
		r4 = self.boundaryPen * jnp.sum(jnp.multiply(Wbdry, jnp.multiply(G, phibdry)))
	...
	#
	# Parameters:
	#		u0, u0x, u0bdry -- relevant values of previous approximation
	#						   u_{i-1}
	#
	# Returns:
	...
	#------------------------------------------------------------------
	def computeError(self, u0, u0x, u0bdry):

		# compute Energy and L2 error
		Energy_k = np.sum(self.wGlobalVal * (u0x - self.exactDx(self.xGlobalVal))**2)
		Energy_k += self.boundaryPen * np.sum(self.wBdry * (u0bdry - self.exactSol(self.xBdry))**2)
		Energy_k = np.sqrt(Energy_k)

		Energy_k0 = np.sum(self.wGlobalVal * (self.exactDx(self.xGlobalVal))**2)
		Energy_k0 += self.boundaryPen * np.sum(self.wBdry * (self.exactSol(self.xBdry))**2)
	...
	def appendBasis(self, params, coeffs, scale):
		...
		gradx = jax.vmap(jax.jacfwd(self.createNetwork, 0), (0, None, None, None), 0)
		phix_i = jnp.reshape(gradx(self.xGlobal, params, coeffs, scale), phi_i.shape)

		self.basis.append(phi_i)
		self.basisx.append(phix_i)
		...
		phixval_i = jnp.reshape(gradx(self.xGlobalVal, params, coeffs, scale), phival_i.shape)

		self.basis_val.append(phival_i)
		self.basisx_val.append(phixval_i)
	...
	#		fTrain -- RHS of PDE
	#		gTrain -- Dirichlet boundary condition
	#
	# Returns:
	...
	#
	#------------------------------------------------------------------
	def galerkinSolve(self, k, fTrain, gTrain):
		...
		for idx in range(k+1):
			for jdx in range(k+1):
				A[idx,jdx] = np.sum(self.wGlobal * (self.basisx[idx]) * (self.basisx[jdx]))
				A[idx,jdx] += self.boundaryPen * np.sum(self.wBdry * (self.basisbdry[idx]) * (self.basisbdry[jdx]))
			b[idx] = np.sum(self.wGlobal * (self.basis[idx]) * (fTrain))
			# b[idx] += self.boundaryPen * np.sum(self.wBdry * (self.basisbdry[idx]) * gTrain)
		...
		uxTrain = np.zeros(self.xGlobal.shape)
		uBdry = np.zeros(self.xBdry.shape)

		uVal = np.zeros(self.xGlobalVal.shape)
		uxVal = np.zeros(self.xGlobalVal.shape)

		for idx in range(k+1):
			uTrain += c[idx] * self.basis[idx]
			uxTrain += c[idx] * self.basisx[idx]
			uBdry += c[idx] * self.basisbdry[idx]

			uVal += c[idx] * self.basis_val[idx]
			uxVal += c[idx] * self.basisx_val[idx]

		return uTrain, uxTrain, uBdry, uVal, uxVal, c
	...
	def update(self, params, coeffs, scalek, U0x, U0bdry, F, G, opt_state, opt_update, get_params):
	...
	def generateBasis(self):
		...
		# while (k < self.maxRef):
			...
			# compute Energy and L2 error
			Energy_k, Energy_k0, L2_k, L2_k0 = self.computeError(uVal, uxVal, uBdry)
			...
			while (np.abs(gradCheck0 - gradCheck1)/np.abs(gradCheck0) > self.gradTol and i < self.maxEpoch):
				...
				params, opt_state, loss = self.update(params, coeffs, scalek, uxTrain, uBdry,
													  fTrain, gTrain, opt_state, opt_update, get_params)
			...
			uTrain, uxTrain, uBdry, uVal, uxVal, c = self.galerkinSolve(k, fTrain, gTrain)
