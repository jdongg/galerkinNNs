...
class GNN:
    ...
    The class is tailored to solving the Stokes equation in the circulator sector with angle pi + arccos(1/sqrt(10)). The singular 
    functions for the extended NN architecture are set up so that the eigenvalue is a trainable parameter. This corresponds to 
    Example 4.2 in [1].
    ...
    def __init__(self, neurons, scale, isExtended: bool, Lx: np.float64, Ly: np.float64, 
                 ...
                 f, u, Du, tolerance, gradTolerance, boundaryPen1, boundaryPen2, boundaryPen3, 
                 H1DivergenceFlag, scale_increment, base_lr, iter_decay_rate, exp_decay_rate,  
    ...
    def initLayer(self, m, n, L, ell, k, numWaves):
        ...
        if (n == 1):
            ...
            alphaOffset1 = random.uniform(-0.35, 0.35)
            alpha = np.reshape(np.linspace(alphaFixedU, 1.047176+1.0, 1), [1, 1]) + alphaOffset1
            ...
            return (Wphi1, bphi1, Wrphi1, alpha, scalephi1, wxm, wym,
                    Wphi2, bphi2, Wrphi2, alpha, scalephi2, wxm, wym,
                    Wq, bq, Wrq, alpha, scaleq, wxm, wym)
        return
    ...
    def separateParams(self, params):
        ...
        for i in range(len(params)):
            (Wphi1, bphi1, Wrphi1, alpha, scalephi1, wxmphi1, wymphi1,
             Wphi2, bphi2, Wrphi2, _, scalephi2, wxmphi2, wymphi2, 
             Wq, bq, Wrq, _, scaleq, wxmq, wymq) = params[i]

            paramsphi1.append((Wphi1, bphi1, Wrphi1, alpha, scalephi1, wxmphi1, wymphi1))
            paramsphi2.append((Wphi2, bphi2, Wrphi2, alpha, scalephi2, wxmphi2, wymphi2))
            paramsq.append((Wq, bq, Wrq, alpha, scaleq, wxmq, wymq))
    ...
    def createNetwork(self, X, Y, params, Cnn, scale, field, Calpha=None):
        ...
        based on eigenfunctions of the corresponding operator pencil. There is only one singular function
        for this problem centered at the origin. See [3] for more details. 
        ...
        """
        # cutoff function
        R = jnp.sqrt(jnp.square(X) + jnp.square(Y))
        offset = (np.pi + np.arccos(1.0/np.sqrt(10.0))) / 2.0
        T = jnp.mod(jnp.arctan2(Y, X) + 2.0*np.pi, 2.0*np.pi) - offset
        R = R.reshape(-1, 1)
        T = T.reshape(-1, 1)
        ...
        # first hidden layer
        W, b, Wr, alpha, scale_NN, wxm, wym = params[0]
        ...
        for W, b, Wr, _, scale_NN, _, _ in params[1:]:
        ...
        if (self.isExtendedNetwork):
            # alpha = np.reshape(np.linspace(alphaFixedU, 1.047176+1.0, 1), [1, 1])
            X = X.reshape(-1, 1)
            Y = Y.reshape(-1, 1)
            ...
            if (field == "u1"):
                ...
                psiT = (jnp.multiply(jnp.multiply(Y, alpha * jnp.cos(alpha * lambda0)), jnp.cos(jnp.multiply(T, alpha-2.0))) +
                        -jnp.multiply(jnp.multiply(X, (alpha - 2.0) * jnp.cos(alpha * lambda0)), jnp.sin(jnp.multiply(T, alpha-2.0))) +
                        -jnp.multiply(jnp.multiply(Y, alpha * jnp.cos((alpha-2.0) * lambda0)), jnp.cos(jnp.multiply(T, alpha))) + 
                        jnp.multiply(jnp.multiply(X, alpha * jnp.cos((alpha-2.0) * lambda0)), jnp.sin(jnp.multiply(T, alpha))))
                Ualpha = jnp.multiply(psiR, psiT) * cutoff
                ...
                lambda0 = (np.pi + np.arccos(1.0/np.sqrt(10.0))) / 2.0
                psiR = jnp.power(R, alpha - 2.0)
                psiT = (-jnp.multiply(jnp.multiply(X, alpha * jnp.cos(alpha * lambda0)), jnp.cos(jnp.multiply(T, alpha-2.0))) +
                        -jnp.multiply(jnp.multiply(Y, (alpha - 2.0) * jnp.cos(alpha * lambda0)), jnp.sin(jnp.multiply(T, alpha-2.0))) +
                         jnp.multiply(jnp.multiply(X, alpha * jnp.cos((alpha-2.0) * lambda0)), jnp.cos(jnp.multiply(T, alpha))) + 
        ...
        if (self.isExtendedNetwork):
            ...
            if (field == "u1"):
                ...
                psiT = (jnp.multiply(jnp.multiply(Y, alpha * jnp.cos(alpha * lambda0)), jnp.cos(jnp.multiply(T, alpha-2.0))) +
                        ...
                        -jnp.multiply(jnp.multiply(Y, alpha * jnp.cos((alpha-2.0) * lambda0)), jnp.cos(jnp.multiply(T, alpha))) + 
                        jnp.multiply(jnp.multiply(X, alpha * jnp.cos((alpha-2.0) * lambda0)), jnp.sin(jnp.multiply(T, alpha))))
                Ualpha1 = jnp.multiply(psiR, psiT) * cutoff
            ...
            Hnn = jnp.column_stack((Hnn, Ualpha1))
        return Hnn
    ...
    def computeLoss(self, params, k, 
        ...
        if (self.isExtendedNetwork):
            ...
            Calpha["leftcorner"] = {}
            for field in self.solution_fields:
                Calpha["leftcorner"][field] = np.reshape(activationcoeffs_singular[field][0 : 1], [1, 1])
        ...
        if (self.isExtendedNetwork):
            resWeight = jnp.power(jnp.sqrt(jnp.power(X, 2) + jnp.power(Y, 2)), self.beta)
            resWeight0 = resWeight
            resWeightBdry = jnp.power(jnp.sqrt(jnp.power(Xbdry, 2) + jnp.power(Ybdry, 2)), self.beta)
        else:
            resWeight = 1.0
            resWeight0 = 1.0
            resWeightBdry = 1.0
    ...
    def generateBasis(self):
        ...
        while (k < self.maxRef):
            ...
            while (np.abs(gradCheck0 - gradCheck1)/np.abs(gradCheck0) > self.gradTol and i < self.maxEpoch):
                ...
                if (self.isExtendedNetwork):
                    ...
                    activationcoeffs_singular["u1"] = np.reshape(coeffsphi1reg[sizesk[-1]:], [1, 1])
                    activationcoeffs_singular["u2"] = np.reshape(coeffsphi2reg[sizesk[-1]:], [1, 1])
                    activationcoeffs_singular["p"] = np.reshape(coeffsqreg[sizesk[-1]:], [1, 1])
                else:
                ...
                _, _, _, alpha, _, _, _ = params_phi1[0]
                self.lambda_epoch.append(alpha.flatten())

                print("Eigenvalue: ", alpha, "Exact: ", alphaFixedU)
            ...
            plt.figure()
            plt.plot(self.lambda_epoch, label="Approximate")
            plt.plot(alphaFixedU * np.ones([len(self.lambda_epoch)]), label="Exact")
            plt.xlabel("Epoch")
            plt.ylabel("$\lambda$")
            plt.grid()
        ...
        pickle.dump(mdic, open(self.RESULTS_PATH + "/saved_stokes2D_singularitylearning_results.pkl", "wb"))
