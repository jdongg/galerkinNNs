...
# leading eigenvalue for Stokes operator pencil problem in the T-shaped channel
...
class GNN:
    ...
    """
    def __init__(self, neurons, scale, isExtended: bool, withCutoff: bool, Lx: np.float64, Ly: np.float64, 
        ...
        # presently, use first three eigenfunctions
        self.isExtendedNetwork = isExtended
        self.withCutoff = withCutoff
        self.alphaSize = 3
    ...
    def point_on_triangle(self, pt1, pt2, pt3):
    ...
    def boxInit(self, m, n, flag, ki, L, ell, numWaves):
        ...
        """
        # first layer (Fourier feature map input)
    ...
    def initLayer(self, m, n, L, ell, k, numWaves):
        ...
        if (n == 1):
            ...
            return (Wphi1, bphi1, Wrphi1, scalephi1, wxm, wym,
                    Wphi2, bphi2, Wrphi2, scalephi2, wxm, wym,
                    Wq, bq, Wrq, scaleq, wxm, wym)
        return
    ...
    def separateParams(self, params):
        ...
        for i in range(len(params)):
            (Wphi1, bphi1, Wrphi1, scalephi1, wxmphi1, wymphi1,
             Wphi2, bphi2, Wrphi2, scalephi2, wxmphi2, wymphi2, 
             Wq, bq, Wrq, scaleq, wxmq, wymq) = params[i]

            paramsphi1.append((Wphi1, bphi1, Wrphi1, scalephi1, wxmphi1, wymphi1))
            paramsphi2.append((Wphi2, bphi2, Wrphi2, scalephi2, wxmphi2, wymphi2))
            paramsq.append((Wq, bq, Wrq, scaleq, wxmq, wymq))
    ...
    def createNetwork(self, X, Y, params, Cnn, scale, field, Calpha=None):
        ...
        based on eigenfunctions of the corresponding operator pencil. There are three sets of singular functions
        for this problem: at both non-convex corners and at the bottom of the cavity corresponding to the Moffatt
        eddies. See [3] for more details. 
        ...
        for W, b, Wr, scale_NN, _, _ in params[1:]:
        ...
        if (self.isExtendedNetwork):
            alpha = np.column_stack(([[alphaFixedU]], [[2.0]], [[2.047176]]))
            X = X.reshape(-1, 1)
            Y = Y.reshape(-1, 1)
        ...
        if (self.isExtendedNetwork):
            ...
            if (field == "u1"):
                ...
                if (self.withCutoff):
                    ...
                    Ualpha1 = jnp.multiply(psiR, psiT)
                ...
                if (self.withCutoff):
                    ...
                    Ualpha2 = jnp.multiply(psiR, psiT)
                ...
                if (self.withCutoff):
                    ...
                    Ualpha3 = jnp.column_stack((H1, H2))
    ...
    def activMatrix(self, i, k, NN, solution0,
        ...
        if (self.isExtendedNetwork):
            ...
            # w0 = w
            w0 = np.sqrt((xTrain-1.0)**2 + (yTrain)**2)**(self.beta-1) * np.sqrt((xTrain+1.0)**2 + (yTrain)**2)**(self.beta-1)
    ...
    def activMatrix2(self, i, k, NN, solution0, C, Fq, DATA):
        ...
        # C[i,:] += np.squeeze(self.boundaryPen1 * NNcompat_idx * NNcompat_jdx)
    ...
    def galerkinUpdate(self, k, neurons, params, solution0, scale, DATA):
        ...
        # when using the extended NN architecture for this problem, the Moffatt eddy functions are
        # treated differently from the other singular functions and activation functions. to better 
        # preserve the shape of the eddy functions, their linear/activation coefficients are coupled
        # together for the variables u1 and u2. E.g. the basis functions for u1 and u2 are
        #
        #           Ψ_1(x) = Σ_j c1_j * σ(W1_j * x + b1_j) + Σ_k d1_j * Ψ1_j(x) + C * Υ(x)
        #           Ψ_2(x) = Σ_j c2_j * σ(W2_j * x + b2_j) + Σ_k d2_j * Ψ2_j(x) + C * Υ(x)
        #
        # where c1 and c2 are the activation coefficients for the regular DNN part of u1 and u2, respectively,
        # d1 and d2 are the activation coefficients for the singular functions corresponding to the two non-convex
        # corners of the domain for u1 and u2, respectively, and C is the coupled activation coefficient
        # for the eddy function Υ which is *shared* between u1 and u2. 
        if self.isExtendedNetwork:
            start = 0 
    ...
    def computeLoss(self, params, k, 
        ...
        if (self.isExtendedNetwork):
            ...
            resWeight0 = jnp.power(jnp.sqrt(jnp.power(X-1.0, 2) + jnp.power(Y, 2)), self.beta - 1) * jnp.power(jnp.sqrt(jnp.power(X+1.0, 2) + jnp.power(Y, 2)), self.beta - 1)
    ...
    def computeError(self, solution0,):
        ...
        if (self.isExtendedNetwork):
            ...
            # resWeight0 = resWeight 
            resWeight0 = np.sqrt((xVal-1.0)**2 + (yVal)**2)**(self.beta - 1) * np.sqrt((xVal+1.0)**2 + (yVal)**2)**(self.beta - 1)
        ...
        u1xexact, u1yexact, u2xexact, u2yexact, pxexact, pyexact = self.exactDx(xVal, yVal)
    ...
    def galerkinSolve(self, k):
        ...
        if (self.isExtendedNetwork):
            ...
            # w0 = w 
            w0 = np.squeeze(np.sqrt((xTrain-1.0)**2 + (yTrain)**2)**(self.beta-1) * np.sqrt((xTrain+1.0)**2 + (yTrain)**2)**(self.beta-1))
    ...
    def generateBasis(self):
        ...
        while (k < self.maxRef):
            ...
            myTitle = r"Exact error $u1-u1_{i-1}$"
            myPath = self.RESULTS_PATH + "/exact_error_u1_" + str(k) + ".png"
            self.plotSurface(k, xPlot, yPlot, ERROR_U1, myTitle, myPath)
            ...
            myTitle = r"Exact error $u2-u2_{i-1}$"
            myPath = self.RESULTS_PATH + "/exact_error_u2_" + str(k) + ".png"
            self.plotSurface(k, xPlot, yPlot, ERROR_U2, myTitle, myPath)
            ...
            myTitle = r"Exact error $p-p_{i-1}$"
            myPath = self.RESULTS_PATH + "/exact_error_p_" + str(k) + ".png"
            self.plotSurface(k, xPlot, yPlot, ERROR_P, myTitle, myPath)
            ...
            # self.natEnergy_iter.append(natEnergy_k)
        ...
        pickle.dump(mdic, open(self.RESULTS_PATH + "/saved_stokes2D_results.pkl", "wb"))
